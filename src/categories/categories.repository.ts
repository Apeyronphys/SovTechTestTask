...
export class CategoriesRepository {
  ...
  async updateCategory(
    ...
    try {
      await this.categoriesRepository.update(id, updateFields);
  ...
  async getManyCatigories(request: IGetCategories): Promise<CategoryEntity[]> {
    try {
      ...
      if (request.active) {
        categories.andWhere('category.active = :active', {
      ...
      if (!request.search) {
        ...
        if (request.description) {
          categories.andWhere(description, {
